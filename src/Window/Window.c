/*
 * Language bindings to CSFML library: Window.c
 * version 2.5.1
 *
...
HB_FUNC( SFWINDOW_CREATEUNICODE )
{
       ...
       ( pItem2 = hb_param( 4, HB_IT_ARRAY ) ) != NULL && hb_arrayLen( pItem2 ) == 7 )
   {
      ...
      hb_sfWindow_ret( sfWindow_createUnicode( mode, ( const sfUint32* ) hb_parstr_utf8( 2, &pText, NULL ), ( unsigned int ) hb_parni( 3 ), &settings ) );
      hb_strfree( pText );
   }
   else
   {
...
HB_FUNC( SFWINDOW_POLLEVENT )
{
   ...
   if( window && hb_param( 2, HB_IT_BYREF ) != NULL )
   {
      sfEvent event;
...
HB_FUNC( SFWINDOW_WAITEVENT )
{
   ...
   if( window && hb_param( 2, HB_IT_BYREF ) != NULL )
   {
      ...
      hb_retl( sfWindow_waitEvent( window, &event ) );
      hb_stornint( event.type, 2 );
   }
   else
   {
...
HB_FUNC( SFWINDOW_SETICON )
{
   ...
   {
      sfWindow_setIcon( window, ( unsigned int ) hb_parni( 2 ), ( unsigned int ) hb_parni( 3 ), ( const sfUint8 * ) HB_UNCONST( hb_parc( 4 ) ) );
   }
   else
   {
      hb_errRT_BASE_SubstR( EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );
